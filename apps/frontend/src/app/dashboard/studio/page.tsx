...
export default function StudioPage() {
  ...
  async function downloadVideoCanvas(result: { id: string; name: string; config: ClipConfig }, clipIndex: number) {
    ...
    // ── Backend render: node-canvas + FFmpeg, 100% fidelity, no browser encoding ──
    const apiRoot = process.env.NEXT_PUBLIC_API_URL ?? "https://api.escalium.io/api/v1";
    const session = await getSession();
    const token = (session as { accessToken?: string } | null)?.accessToken;
    ...
    try {
      const resp = await fetch(`${apiRoot}/media/render-video`, {
        method: "POST",
      ...
      if (!resp.ok) {
        ...
        throw new Error((err as { detail?: string }).detail ?? `Render failed (${resp.status})`);
      }
      const blob = await resp.blob();
      const dlUrl = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = dlUrl; a.download = `${result.name}.mp4`;
